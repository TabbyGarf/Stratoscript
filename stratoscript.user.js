...
// @name         Stratoscript
// @version      1.14.22.1
// @description  1.14.22.1 > Ajout du mode VIM, par Tigriz.
...
function addNoelshackButton() {
    ...
    formGroups.forEach(formGroup => {
        ...
        noelshackLogo.src = 'https://i.imgur.com/au6766C.png'; // Replace with the actual path to the Noelshack logo
    ...
    function ajoutLecteursEtIntegrations(container) {
        ...
        document.querySelectorAll( '.message-content a' ).forEach( async function ( e ) {
            ...
            if ( parametres[ "sw-corr-url-odysee" ] == true ) {
                ...
                if (parametres["sw-rbc"] == true) {
                    function embedImagesFromRisibankLinks(container) {
                        // Select all anchor elements
                        let links = document.querySelectorAll('a[href^="https://risibank.fr/cache/medias/"]');

                        // Iterate over the selected anchor elements
                        links.forEach(function(link) {
                            // Get the href attribute value of the current link
                            let href = link.getAttribute('href');
                            // Create an image element
                            let img = document.createElement('img');
                            // Set the source attribute to the href of the link
                            img.src = href;
                            // Set width and height attributes as needed
                            img.width = 68;
                            img.height = 51;
                            // Replace the link with the embedded image
                            link.parentNode.replaceChild(img, link);
                        });
                    }

                    // Call the function to embed images from risibank links
                    embedImagesFromRisibankLinks(document.body);
    ...
    function majPannel_Parametres() {
        ...
        document.getElementById( 'sw-rbc' ).querySelector( 'input' ).checked = parametres[ "sw-rbc" ];
        // Liste des topics
        ...
        document.getElementById( 'sw-vim' ).querySelector( 'input' ).checked = parametres[ "sw-vim" ];
    }
    ...
    function creerPannelStratoscript() {
        ...
        document.querySelectorAll( '#btn-validation-parametres' ).forEach( function ( e ) {
            e.onclick = function () {
                ...
                parametres[ "sw-rbc" ] = document.getElementById( 'sw-rbc' ).querySelector( 'input' ).checked;
                ...
                parametres[ "sw-vim" ] = document.getElementById( 'sw-vim' ).querySelector( 'input' ).checked;
    ...
    function vim() {
      ...
      document.onkeydown = async (e) => {
        ...
        switch(e.key) {
          ...
          case 'R':
          case 'F5':
            if(path.startsWith( "/forum" )) refreshTopics()
        ...
        document.onkeyup = async (e) => {
          ...
          switch(e.key) {
            case 'Alt':
            case 'AltGraph':
              document.querySelectorAll('.ss-vim-hint').forEach((el)=>el.remove())
          }
        }
