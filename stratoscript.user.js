...
// @version      1.14.10.2
// @description  1.14.10.2 > Ajout verif màj
...
( function () {
    ...
    const rawScriptUrl = 'https://github.com/TabbyGarf/Stratoscript/raw/main/stratoscript.user.js';
    ...
    fetch(rawScriptUrl)
        ...
        if (rawScriptVersion && !compareVersions(installedVersion, rawScriptVersion)) {
            // Versions don't match, add an update image next to the button
            addUpdateImage();
        }
    })
        .catch(error => console.error('Error fetching script file:', error));

    // Function to add an update image link next to the button
    function addUpdateImage() {
        // Create an anchor element for the update link
        const updateLink = document.createElement('a');
        updateLink.href = 'https://github.com/TabbyGarf/Stratoscript/raw/main/stratoscript.user.js';
        updateLink.target = '_blank'; // Open link in a new tab (adjust as needed)

        // Create an image element for the update
        const updateImage = document.createElement('img');
        updateImage.src = 'https://cdn.discordapp.com/attachments/592805019590459403/1199570344990740510/update.png'; // Replace with the URL of your update image
        updateImage.height = 24;
        updateImage.alt = 'Une mise à jour est disponible !';
        updateImage.style.marginLeft = '5px'; // Adjust the margin as needed

        // Append the image to the anchor element
        updateLink.appendChild(updateImage);

        // Get the btnStratoscript element
        const btnStratoscript = document.querySelector('.btnStratoscript');

        // Insert the update link next to the btnStratoscript button
        btnStratoscript.parentNode.insertBefore(updateLink, btnStratoscript.nextSibling);
    }
    /* ==========================================================
    |                                                           |
    |                      INITIALISATION                       |
...
function appliquer_blacklist_kw(page) {
    ...
    page.querySelectorAll('.topics > tbody > tr').forEach((e) => {
        ...
        if (e.querySelector('.topics-title a')) {
            ...
            if (blacklist_kw.some(keywordObj => {
                ...
                console.log("Title Includes Keyword?", title.includes(keyword));
    ...
    function creerPannelStratoscript() {
        ...
        boutonScript.innerHTML = '<a style="height:70px;width:55px" class="btnStratoscript" data-toggle="modal" data-target="#modalStratoscript" href="#stratoscriptPanel" ><img class="btnStratoscript" style="position:absolute" target="_blank" src="https://media.discordapp.net/attachments/592805019590459403/1108591534594596965/Untitled.png" alt="Stratoscript" height="24"></a>';
